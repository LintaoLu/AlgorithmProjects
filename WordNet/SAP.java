...
public class SAP {
    ...
    private int[] shortestPath(Iterable<Integer> container) {
        int[] dis = new int[G.V()];
        Arrays.fill(dis, Integer.MAX_VALUE);
        Queue<Integer> queue = new LinkedList<>();
        for (int i : container) queue.offer(i);
        int step = 0;
        while (!queue.isEmpty()) {
            int size = queue.size();
    ...
    private int[] getLCA(Iterable<Integer> v, Iterable<Integer> w) {
        ...
        int[] dis1 = shortestPath(v);
        int[] dis2 = shortestPath(w);
        int[] res = { max, -1 };
        for (int i = 0; i < G.V(); i++) {
